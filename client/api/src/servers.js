export class ServerApi {
  ...
  async create(data) {
    const c = (crypto || {}).getRandomValues
      ? crypto
      : ((crypto || {}).webcrypto || {}).getRandomValues
      ? crypto.webcrypto.getRandomValues
      : undefined;
    if (c === undefined) throw new Error("no suitable crypto found");
    const id = Array.from(c.getRandomValues(new Uint8Array(4)))
  ...
  async fileExists(id, path) {
    if (path.indexOf("/") === 0) path = path.substring(1);
    try {
      // Use _buildUrl instead of manual concatenation
      const url = this._api._buildUrl(this.getFileUrl(id, path));
      const res = await this._api._axios.get(url);
  ...
  async createFolder(id, path) {
    if (path.indexOf("/") === 0) path = path.substring(1);
    await this._api.put(this.getFileUrl(id, path), undefined, { folder: true });
    return true;
  }
  ...
  async extractFile(id, path, destination) {
    ...
    await this._api.post(`/api/servers/${id}/extract/${path}`, undefined, {
      destination,
    });
    return true;
  }
  ...
  async createBackup(id, name) {
    await this._api.post(`/api/servers/${id}/backup/create`, undefined, {
      name,
    });
    return true;
  }
...
class Server {
  ...
  constructor(api, serverData, basePath = "") {
    ...
    this._api = api;
    this._basePath = basePath || api._basePath || ""; // ← ADD THIS LINE
    this._openSocket();
    this.emit("open");
  }
  ...
  _openSocket() {
    ...
    if (basePath && basePath !== "") {
      ...
      wsPath = cleanBasePath + wsPath;
    }

    this._socket = new WebSocket(`${protocol}://${host}${wsPath}`);
  ...
  async uploadFile(path, content, onUploadProgress) {
    return await this._api.server.uploadFile(
      this.id,
      path,
      content,
      onUploadProgress
